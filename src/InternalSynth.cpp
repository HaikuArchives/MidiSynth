...
CInternalSynth::~CInternalSynth() { delete m_midiSynth; }

// ---------------------------------------------------------------------------
// Accessors

BBitmap *CInternalSynth::GetIcon(icon_size size) const {
...
void CInternalSynth::Init() {
  if (m_midiSynth == NULL) {
    m_midiSynth = new BMidiSynth;
  }
}

bool CInternalSynth::LoadSynthData(entry_ref *ref) {
...
bool CInternalSynth::LoadSynthData(void) {
...
void CInternalSynth::NoteOff(uchar channel, uchar note, uchar velocity,
                             bigtime_t time) {
  m_midiSynth->NoteOff(channel + 1, note, velocity, time / 1000);
}

void CInternalSynth::NoteOn(uchar channel, uchar note, uchar velocity,
                            bigtime_t time) {
  m_midiSynth->NoteOn(channel + 1, note, velocity, time / 1000);
}

void CInternalSynth::KeyPressure(uchar channel, uchar note, uchar pressure,
                                 bigtime_t time) {
  m_midiSynth->KeyPressure(channel + 1, note, pressure, time / 1000);
}

void CInternalSynth::ControlChange(uchar channel, uchar controlNumber,
                                   uchar controlValue, bigtime_t time) {
  m_midiSynth->ControlChange(channel + 1, controlNumber, controlValue,
                             time / 1000);
}

void CInternalSynth::ProgramChange(uchar channel, uchar programNumber,
                                   bigtime_t time) {
  m_midiSynth->ProgramChange(channel + 1, programNumber, time / 1000);
}

void CInternalSynth::ChannelPressure(uchar channel, uchar pressure,
                                     bigtime_t time) {
  m_midiSynth->ChannelPressure(channel + 1, pressure, time / 1000);
}

void CInternalSynth::PitchBend(uchar channel, uchar lsb, uchar msb,
                               bigtime_t time) {
