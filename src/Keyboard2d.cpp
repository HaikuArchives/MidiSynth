...
void
Keyboard2D::FrameResized(float width, float height)
{
	LayoutChanged();
...
void
Keyboard2D::DrawKey(uchar key, uchar on, BRect& rect)
{
	BRect r;
	...
	switch (key) {
		...
		case 5:
			...
			r.Set(rect.left + 1, rect.top + 1, rect.right - dxB - 1,
				rect.bottom - 1);
			FillRect(r);
			...
			AddLine(p, p = BPoint(rect.right - dxB, rect.top + dyB),
				outlineColor[0]);
		...
		case 10:
			...
			AddLine(p, p = BPoint(rect.left + dxW, rect.top + dyW),
				outlineColor[0]);
			AddLine(p, p = BPoint(rect.left - dxW, rect.top + dyW),
				outlineColor[1]);
		...
		case 9:
			SetHighColor(whiteKeyColor[on]);
			r.Set(rect.left + dxB, rect.top + 1, rect.right - dxB - 1,
				rect.top + dyB);
			FillRect(r);

			r.Set(rect.left + 1, rect.top + dyB + 1, rect.right - 1,
				rect.bottom - 1);
			FillRect(r);
			...
			AddLine(p, p = BPoint(rect.left + dxW, rect.top + dyB),
				outlineColor[1]);
			AddLine(p, p = BPoint(rect.left + dxW, rect.top), outlineColor[1]);
			EndLineArray();
			rect.OffsetBy(rect.Width(), 0);
		...
		case 11:
			SetHighColor(whiteKeyColor[on]);
			r.Set(
				rect.left + dxB, rect.top + 1, rect.right - 1, rect.bottom - 1);
			FillRect(r);

			r.Set(rect.left + 1, rect.top + dyB + 1, rect.left + dxB,
				rect.bottom - 1);
			FillRect(r);

			BeginLineArray(6);
			AddLine(BPoint(rect.left + dxW, rect.top),
				p = BPoint(rect.right, rect.top), outlineColor[0]);
			AddLine(p, p = BPoint(rect.right, rect.bottom), outlineColor[0]);
			AddLine(p, p = BPoint(rect.left, rect.bottom), outlineColor[1]);
			AddLine(p, p = BPoint(rect.left, rect.top + dyB), outlineColor[1]);
			AddLine(p, p = BPoint(rect.left + dxW, rect.top + dyB),
...
Keyboard2D::Draw(BRect updateRect)
{
	...
	if (rows == 1) {
		...
		if (gap > 0) {
			rect.Set(
				updateRect.left, height, updateRect.right, height + gap - 1);
...
bool
Keyboard2D::KeyCode(int32& key, int32& modifiers)
{
...
void
Keyboard2D::KeyDown(const char* bytes, int32 num)
{
	int32 key, modifiers;
...
void
Keyboard2D::KeyUp(const char* bytes, int32 num)
{
	int32 key, modifiers;
	if (KeyCode(key, modifiers))
...
void
Keyboard2D::MessageReceived(BMessage* msg)
{
...
uchar
Keyboard2D::PressedKey(BPoint point)
{
	int note = 0;
...
void
Keyboard2D::MouseDown(BPoint point)
{
	MakeFocus();
...
void
Keyboard2D::MouseUp(BPoint point)
{
	uchar note = PressedKey(point);
	mouseTracking = false;
	if (note == 255)
...
void
Keyboard2D::MouseMoved(BPoint point, uint32 transit, const BMessage* message)
{
