...
View::View(BRect rect, int16 octaves, int16 rows)
  ...
  rect.SetLeftTop(BPoint(0, volumeSlider->Bounds().Height() + 1));
  subViewTop =
      2 + volumeSlider->Bounds().Height() + velocitySlider->Bounds().Height();
  rect.top = subViewTop;
...
void View::FocusKeyboard() { ((Keyboard2D *)keyboard)->MakeFocus(true); }
...
Keyboard *View::GetKeyboard() { return (Keyboard2D *)keyboard; }
...
double View::GetMainVolume() { return volumeSlider->Value() / 100.0; }

void View::VelocityChanged() { SetVelocity(channel, velocitySlider->Value()); }
...
void View::NoteOn(uchar key, uchar source, bool map) {
  ...
  if (!old) {
    midiOut->SprayNoteOn(channel, note, channels[channel].velocity,
                         system_time());
  ...
  if (chord.enabled && (old != notePlaying[note].device)) {
    ...
    for (int i = 0; i < chord.len; i++) {
      ...
      notePlaying[n].count++;
      if (!old) {
        midiOut->SprayNoteOn(channel, n, channels[channel].velocity,
                             system_time());
        keyboard->NoteOn(n);
      }
    }
...
void View::NoteOff(uchar key, uchar source, bool map) {
  ...
  if (!notePlaying[note].device) {
    midiOut->SprayNoteOff(channel, note, channels[channel].velocity,
                          system_time());
  ...
  if (chord.enabled && (old != notePlaying[note].device)) {
    ...
    for (int i = 0; i < chord.len; i++) {
      ...
      if (!notePlaying[n].device) {
        midiOut->SprayNoteOff(channel, n, channels[channel].velocity,
                              system_time());
        keyboard->NoteOff(n);
      }
    }
...
// Key Table
KeyTable *View::GetKeyTable() { return &keyTable; }
...
void View::EndRemapKeys() { keys.remap = false; }
...
void View::ReleaseNotes(uchar source) {
  for (int i = 0; i < 128; i++) {
    ...
    if (!notePlaying[i].device) {
      midiOut->SprayNoteOff(channel, i, channels[channel].velocity,
                            system_time());
      keyboard->NoteOff(i);
    }
  }
...
void View::SetChord(const char *chord) {
  ...
  for (i = 0; (i < 255) && (chord[i] != '\0'); i++) {
    if (i == 0)
      ...
      this->chord.notes[i] = this->chord.notes[i - 1] + DecodeChord(chord[i]);
  }
...
void View::ControlChange(uchar channel, uchar controlNumber, uchar controlValue,
                         bigtime_t time) {
...
void View::KeyPressure(uchar channel, uchar note, uchar pressure,
                       bigtime_t time) {
...
void View::SystemCommon(uchar statusByte, uchar data1, uchar data2,
                        bigtime_t time) {
