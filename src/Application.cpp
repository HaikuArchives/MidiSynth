...
static char *instrGroups[INSTR_GROUPS] = {"Pianos",
                                          "Tuned Idiophones",
                                          "Organs",
                                          "Guitars",
                                          "Basses",
...
static struct {
  uchar from, to;
} instrIndizes[INSTR_GROUPS] = {{0, 7},    {8, 15},    {16, 23},   {24, 31},
...
static void AddMenuItem(BMenu *menu, const char *name, int32 what, int32 data,
                        int32 current);
...
AppWindow::AppWindow(BRect aRect)
  ...
  menu->AddItem(scopeMenu =
                    new BMenuItem("Scope", new BMessage(MENU_SCOPE), 'V'));
  ...
  for (i = 0; i < INSTR_GROUPS; i++) {
    menu->AddItem(
        item = new BMenuItem(instrGroups[i], new BMessage(MENU_GROUPS + i)));
    if (i == 0)
      item->SetMarked(true);
  }
  menu->SetRadioMode(true);
  ...
  for (i = 1; i <= 11; i++) {
    sprintf(string, "%d", i);
    menu->AddItem(item =
                      new BMenuItem(string, new BMessage(MENU_OCTAVE + i - 1)));
    if (i == 4)
      item->SetMarked(true);
  }
  menubar->AddItem(menu);
...
static const char *patches[5] = {"Patches300.hsb", "Patches111.hsb",
                                 "Patches.hsb", "big_synth.sy", NULL};
#endif
...
bool AppWindow::OnProgramChanged(BMessage *message) {
  if ((message->what >= MENU_INSTRUMENTS) &&
    ...
    view->ProgramChange(view->GetChannel(), message->what - MENU_INSTRUMENTS,
                        system_time());
    return true;
  }
  return false;
...
void AppWindow::LoadKeyMap(const char *map) {
  ...
  if (file.InitCheck() == B_OK) {
    if (!view->GetKeyTable()->Read(file)) {
      BAlert *help =
          new BAlert("File Error", "This is no valid key map file.", "Ok");
      help->Go();
...
void AppWindow::BuildKeyboardRowsMenu(BMenu *menu) {
  ...
  for (int i = 1; i <= 2; i++) {
    sprintf(string, "%d", i);
    menu->AddItem(item =
                      new BMenuItem(string, NewMessage(MENU_KEYBOARD_ROWS, i)));
...
void AppWindow::FrameResized(float w, float h) { settings.SetWindowSize(w, h); }
