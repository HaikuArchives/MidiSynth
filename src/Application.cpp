...
// Pianos (0-7)
"Acoustic Grand", "Bright Grand", "Electric Grand",
"Honky Tonk", "Electric Piano", "Electric Piano 2", 
...
"Synth Strings 2", "Voice Aah", "Voice Ooh", "Synth Voice",
"Orchestra Hit",
// Brass (56-63)
...
static char *instrGroups[INSTR_GROUPS] = {"Pianos", "Tuned Idiophones", "Organs", 
	"Guitars", "Basses" , "Strings and Timpani", "Ensemble Strings and Voices", 
	"Brass", "Reeds", "Pipes", "Synth Leads", "Synth Pads", "Musical Effects", 
	"Ethnic", "Percussion", "Sound Effects"};

static struct { uchar from, to; } instrIndizes[INSTR_GROUPS] = {{0, 7}, {8, 15}, {16, 23},
	{24, 31}, {32, 39}, {40, 47}, {48, 55}, {56, 63}, {64, 71}, {72, 79}, 
	{80, 87}, {88, 95}, {96, 103}, {104, 111}, {112, 119}, {120, 127}};

// function prototypes
static BMessage * NewMessage(int32 what, int32 data);
static void AddMenuItem(BMenu *menu, const char *name, int32 what, int32 data, int32 current);
...
BMenuItem *item = new BMenuItem(name, NewMessage(what, data));
	menu->AddItem(item);
	if (data == current) item->SetMarked(true);
}
...
AppWindow::AppWindow(BRect aRect) 
	...
	if (B_OK == be_app->GetAppInfo(&ai)) {
		...
		if (B_OK != keyMapPath.SetTo(&path, "KeyMappings")) keyMapPath = path;
	...
	midiSynthEnabled = false; remapKeys = false;
	...
	menu->AddItem(scopeMenu = new BMenuItem("Scope", new BMessage(MENU_SCOPE), 'V')); 
	...
	menu->AddItem(new BMenuItem("Remap Keys", new BMessage(MENU_REMAP_KEYS), 'R'));
	menu->AddSeparatorItem();
	// Keyboard	
	...
	for (i = 1; i <= 16; i++) {
		if (i == 10)
			...
			sprintf(string, "%d", i);
		menu->AddItem(item = new BMenuItem(string, new BMessage(MENU_CHANNEL+i-1)));
	...
	for (i = 0; i < INSTR_GROUPS; i++) {
		menu->AddItem(item = new BMenuItem(instrGroups[i], new BMessage(MENU_GROUPS+i)));
	...
	for (i = 0; i < INSTR_GROUPS; i++) {
		...
		for (j = instrIndizes[i].from; j <= instrIndizes[i].to; j++) {
			...
			if (j == instrIndizes[i].from) item->SetMarked(true);
		}
		menu->SetRadioMode(true);
	}
	...
	for (i = 1; i <= 11; i++) {
		sprintf(string, "%d", i);
		menu->AddItem(item = new BMenuItem(string, new BMessage(MENU_OCTAVE+i-1)));
		if (i == 4) item->SetMarked(true);
	}
	menubar->AddItem(menu);
/*  // Keyboard
	menu = new BMenu("Keyboard");
	menu->SetRadioMode(true);
	menu->AddItem(item = new BMenuItem("2D", new BMessage(MENU_KEYBOARD_2D)));
	item->SetMarked(true);
	menu->AddItem(item = new BMenuItem("3D", new BMessage(MENU_KEYBOARD_3D)));
	menubar->AddItem(menu);	
*/
	// Port
	...
	// Chords
	chordMenu = menu = new BMenu("Chord"); menu->SetRadioMode(true);
	...
	for (i = 0; i < 16; i++) view->ProgramChange(i, B_ACOUSTIC_GRAND, system_time());
...
// Midi Port
void AppWindow::PopulatePortMenus()
{
...
void AppWindow::SetPatchesMenu(BMenu *menu) {
	int16 i = 0;
	BEntry *e;
...
void AppWindow::OnPatchSelected(BMessage *msg) {
	int16 i;
...
BEntry *AppWindow::FindPatch(const char *patch) {
	int16 i = 0;
	BEntry *e = NULL;
	char name[B_FILE_NAME_LENGTH];
	while (NULL != (e = (BEntry*)synthEntries.ItemAt(i))) {
		e->GetName(name);
		if (strcmp(name, patch) == 0) return e;
		i++;
	}
	return NULL;
}
...
bool AppWindow::LoadPatch(BEntry *entry) {
	entry_ref e;
	bool ok = false;
	...
	if (ok) {		
		...
		//}
		be_synth->SetVoiceLimits(settings.GetMaxSynthVoices(),
								0,
#ifndef __HAIKU__
...
void AppWindow::LoadChords(BMenu *menu) {
app_info ai;
	if (B_OK == be_app->GetAppInfo(&ai)) {
...
void AppWindow::EnableMidiSynth(bool enable) {
...
void AppWindow::ChangeInstrumentMenu(int group) {
...
void AppWindow::OnChordSelected() {
...
bool AppWindow::OnInstrumentChanged(BMessage *message) {
	if ((message->what >= MENU_GROUPS) && 
		(message->what < MENU_GROUPS+INSTR_GROUPS)) {
		BMenu *menu = instrumentMenu[message->what - MENU_GROUPS];
		BMenuItem *item = menu->FindMarked();
		if (item != NULL) PostMessage(item->Message());
		return true;
	}
	return false; 
}	

bool AppWindow::OnProgramChanged(BMessage *message) {
	if ((message->what >= MENU_INSTRUMENTS) && 
		(message->what < MENU_INSTRUMENTS+INSTRUMENTS)) {
		view->ProgramChange(view->GetChannel(), message->what-MENU_INSTRUMENTS, system_time());
		return true;
	}
	return false;
}

bool AppWindow::OnOctaveChanged(BMessage *message) {
...
bool AppWindow::OnChannelChanged(BMessage *message) {
	if ((message->what >= MENU_CHANNEL) && (message->what < MENU_CHANNEL+16)) {
		uchar channel = message->what - MENU_CHANNEL;
		view->SetChannel(channel);
		view->AllNotesOff(MIDIIN); view->Invalidate();
		return true;
	}
	return false;
}

void AppWindow::OnSynthesizerEnabled() {
...
void AppWindow::BuildKeyMapMenu(BMenu *menu) {
	BMenuItem *item;
	while (NULL != (item = menu->RemoveItem((int32)0))) delete item;
	...
	if (dir.InitCheck() == B_OK) {
		...
		while (dir.GetNextEntry(&e) == B_OK) {
			e.GetName(name);
			menu->AddItem(item = new BMenuItem((const char*)name, 
										new BMessage(MENU_KEY_MAP_SELECTED)));
...
void AppWindow::LoadKeyMap(const char *map) {
	BFile file(&keyMapPath, map, B_READ_ONLY);
	if (file.InitCheck() == B_OK) {
		if (!view->GetKeyTable()->Read(file)) {
			BAlert *help = new BAlert("File Error", 
				"This is no valid key map file.", "Ok");
			help->Go();
			view->GetKeyTable()->DefaultKeyTable();
		} 
	} else {
		BAlert *help = new BAlert("File Error", 
			"Error opening key map file.", "Ok");
		help->Go();
	}
}

void AppWindow::OnKeyMapSelected(BMessage *msg) {
BMenuItem *item; void *ptr;
...
// Reverb
void AppWindow::BuildReverbMenu(BMenu *menu) {
...
void AppWindow::OnReverb(BMessage *msg) {
int32 data;
...
// SamplingRate
void AppWindow::BuildSamplingRateMenu(BMenu *menu) {
...
void AppWindow::OnSamplingRate(BMessage *msg) {
int32 data;
	if (B_OK == msg->FindInt32("data", &data)) {
...
#ifndef __HAIKU__
// Interpolation
void AppWindow::BuildInterpolationMenu(BMenu *menu) {
int32 current = settings.GetInterpolation();
	menu->SetRadioMode(true);
	AddMenuItem(menu, "Drop Sample", MENU_INTERPOLATION, B_DROP_SAMPLE, current);
	AddMenuItem(menu, "2 Point Interpolation", MENU_INTERPOLATION, B_2_POINT_INTERPOLATION, current);
	AddMenuItem(menu, "Linear Interpolation", MENU_INTERPOLATION, B_LINEAR_INTERPOLATION, current);
}

void AppWindow::OnInterpolation(BMessage *msg) {
int32 data;
	if (B_OK == msg->FindInt32("data", &data)) {
...
// Max Synth Voices
void AppWindow::BuildMaxSynthVoicesMenu(BMenu *menu) {
BMenuItem *item;
char string[4];
	menu->SetRadioMode(true);
	for (int i = 1; i <= 32; i++) {
		sprintf(string, "%d", i);
		menu->AddItem(item = new BMenuItem(string, 
					  NewMessage(MENU_MAX_SYNTH_VOICES, i)));
		if (i == settings.GetMaxSynthVoices()) item->SetMarked(true);
	}
}

void AppWindow::OnMaxSynthVoices(BMessage *msg) {
int32 data;
	if (B_OK == msg->FindInt32("data", &data)) {
...
#ifndef __HAIKU__
// Limiter Threshhold
void AppWindow::BuildLimiterThreshholdMenu(BMenu *menu) {
BMenuItem *item;
char string[4];
	menu->SetRadioMode(true);
	for (int i = 1; i <= 32; i++) {
		sprintf(string, "%d", i);
		menu->AddItem(item = new BMenuItem(string, 
					  NewMessage(MENU_LIMITER_THRESHHOLD, i)));
		if (i == settings.GetLimiterThreshhold()) item->SetMarked(true);
	}
}

void AppWindow::OnLimiterThreshhold(BMessage *msg) {
int32 data;
	if (B_OK == msg->FindInt32("data", &data)) {
...
// Keyboard
void AppWindow::BuildKeyboardOctavesMenu(BMenu *menu) {
BMenuItem *item;
char string[4];
	menu->SetRadioMode(true);
	for (int i = 1; i <= 11; i++) {
		sprintf(string, "%d", i);
		menu->AddItem(item = new BMenuItem(string, 
					  NewMessage(MENU_KEYBOARD_OCTAVES, i)));
		if (i == settings.GetKeyboardOctaves()) item->SetMarked(true);
	}
}

void AppWindow::OnKeyboardOctaves(BMessage *msg) {
int32 data;
	if (B_OK == msg->FindInt32("data", &data)) {
		Keyboard2D *k = (Keyboard2D*)view->GetKeyboard();
		k->SetOctaves(data); k->LayoutChanged();
		if (Lock()) { k->Invalidate(); Unlock(); }
		settings.SetKeyboardOctaves(data);
	}
}

void AppWindow::BuildKeyboardRowsMenu(BMenu *menu) {
BMenuItem *item;
char string[4];
	menu->SetRadioMode(true);
	for (int i = 1; i <= 2; i++) {
		sprintf(string, "%d", i);
		menu->AddItem(item = new BMenuItem(string, 
					  NewMessage(MENU_KEYBOARD_ROWS, i)));
		if (i == settings.GetKeyboardRows()) item->SetMarked(true);
	}
}

void AppWindow::OnKeyboardRows(BMessage *msg) {
int32 data;
	if (B_OK == msg->FindInt32("data", &data)) {
		Keyboard2D *k = (Keyboard2D*)view->GetKeyboard();
		k->SetRows(data); k->LayoutChanged();
		if (Lock()) { k->Invalidate(); Unlock(); }
...
// Scope Window
void AppWindow::OpenScopeWindow() {
	BRect rect; float w, h;
...
void AppWindow::FrameMoved(BPoint p) {
	...
	if (scopeWindow && scopeWindow->Lock()) {
		...
		settings.SetScopeWindowPosition(pos + settings.GetScopeWindowPosition());
	}
...
void AppWindow::MessageReceived(BMessage *message) {
	...
	case MENU_RESET: OnReset();
		break;

	case MENU_KEYBOARD_2D: view->SetKeyboard(0);
		break;

	case MENU_KEYBOARD_3D: view->SetKeyboard(1);
		break;
	...
	break;
	case MENU_SYNTH_ENABLED: OnSynthesizerEnabled();
		break;

	case MSG_PITCH_BEND_CHANGED: view->PitchBendChanged();
		break;

	case MSG_CH_PRESSURE_CHANGED: view->ChannelPressureChanged();
		break;
		
	case MSG_PAN_CHANGED: view->PanChanged();
		break;
		
	case MSG_VOLUME_CHANGED: OnVolumeChanged();
		break;
		
	case MSG_VELOCITY_CHANGED: view->VelocityChanged();
		break;
		
	case MENU_PATCH_SELECTED: OnPatchSelected(message);
		break;
		
	case MENU_CHORD: OnChordSelected();
		break;

	case MENU_REMAP_KEYS: OnRemapKeys();
		break;

	case B_SAVE_REQUESTED: OnSave(message);
		break;

	case MENU_KEY_MAP_SELECTED: OnKeyMapSelected(message);
		break;
	
	case MENU_REVERB: OnReverb(message);
		break;
		
	case MENU_SAMPLING_RATE: OnSamplingRate(message);
		break;
		
	case MENU_MAX_SYNTH_VOICES: OnMaxSynthVoices(message);
		break;

#ifndef __HAIKU__
	case MENU_INTERPOLATION: OnInterpolation(message);
		break;
	case MENU_LIMITER_THRESHHOLD: OnLimiterThreshhold(message);
		break;
#endif
		
	case MENU_KEYBOARD_OCTAVES: OnKeyboardOctaves(message);
		break;

	case MENU_KEYBOARD_ROWS: OnKeyboardRows(message);
		break;
	
	case MSG_SCOPE_CLOSED_NOTIFY: OnScopeWindowClosed();
		break;			
	
	case MENU_SCOPE: OnScope();
		break;
...
bool AppWindow::QuitRequested() {
	...
	menubar->RemoveItem(instrGroup);
	for (int i = 0; i < INSTR_GROUPS; i++) delete instrumentMenu[i];
...
App::App() : BApplication("application/x-vnd.midisynth") {
	BRect aRect;
