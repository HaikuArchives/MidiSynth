...
Scope::Scope(BRect rect)
    :
    BView(rect, NULL, B_FOLLOW_LEFT | B_FOLLOW_TOP,
...
void Scope::Draw(BRect updateRect) { DrawBitmap(mBitmap, BPoint(0, 0)); }
...
void Scope::AttachedToWindow(void) { Window()->SetPulseRate(PULSE_RATE); }
...
void ScopeWindow::Quit() {
  mLooper->PostMessage(MSG_SCOPE_CLOSED_NOTIFY);
  BWindow::Quit();
}

void ScopeWindow::FrameMoved(BPoint p) { mSettings->SetScopeWindowPosition(p); }

void ScopeWindow::FrameResized(float w, float h) {
