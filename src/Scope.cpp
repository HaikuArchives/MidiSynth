...
Scope::Scope(BRect rect) :
	...
	if (mBitmap->Lock()) {
		mOffscreenView = new BView(mBitmap->Bounds(), "", B_FOLLOW_NONE, B_WILL_DRAW);
...
ScopeWindow::ScopeWindow(BLooper *looper, GlobalSettings *settings, BRect rect) 
	: BWindow(rect, "Scope", B_FLOATING_WINDOW_LOOK, B_NORMAL_WINDOW_FEEL, B_NOT_RESIZABLE),
		mSettings(settings), mLooper(looper)
{
...
void ScopeWindow::FrameMoved(BPoint p) {
